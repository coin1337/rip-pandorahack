...
public class CFontRenderer extends CFont {
   ...
   public float drawStringWithShadow(String text, double x, double y, PandoraColor color) {
      float shadowWidth = this.drawString(text, x + 1.0D, y + 1.0D, color, true);
      return Math.max(shadowWidth, this.drawString(text, x, y, color, false));
   }

   public float drawString(String text, float x, float y, PandoraColor color) {
      return this.drawString(text, (double)x, (double)y, color, false);
   }

   public float drawCenteredStringWithShadow(String text, float x, float y, PandoraColor color) {
      return this.drawStringWithShadow(text, (double)(x - (float)(this.getStringWidth(text) / 2)), (double)y, color);
   }

   public float drawCenteredString(String text, float x, float y, PandoraColor color) {
      return this.drawString(text, x - (float)(this.getStringWidth(text) / 2), y, color);
   }

   public float drawString(String text, double x, double y, PandoraColor pandoraColor, boolean shadow) {
      --x;
      y -= 2.0D;
      PandoraColor color = new PandoraColor(pandoraColor);
      if (text == null) {
         return 0.0F;
      } else {
         if (color.getRed() == 255 && color.getGreen() == 255 && color.getBlue() == 255 && color.getAlpha() == 32) {
            color = new PandoraColor(255, 255, 255);
         }

         if (color.getAlpha() < 4) {
            color = new PandoraColor(color, 255);
         }

         if (shadow) {
            color = new PandoraColor(color.getRed() / 4, color.getGreen() / 4, color.getBlue() / 4, color.getAlpha());
         }
